#!/usr/bin/env bash
# Runs all seven suites. install.sh is checked against temporary target
...
STATUS=0
for suite in "$HERE/test_install.sh" "$HERE/test_resweep.sh" "$HERE/test_surfaces.sh" "$HERE/test_recheck.sh" "$HERE/test_against_ceetrix.sh" "$HERE/test_rename.sh" "$HERE/test_plugin_payload.sh"; do
  printf '\n=== %s ===\n' "$(basename "$suite")"
