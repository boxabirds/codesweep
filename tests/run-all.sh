#!/usr/bin/env bash
# Runs all six suites. install.sh is checked against temporary target
...
# proves the change of name altered nothing else. None substitutes for another.
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
STATUS=0
for suite in "$HERE/test_install.sh" "$HERE/test_resweep.sh" "$HERE/test_surfaces.sh" "$HERE/test_recheck.sh" "$HERE/test_against_ceetrix.sh" "$HERE/test_rename.sh"; do
  printf '\n=== %s ===\n' "$(basename "$suite")"
